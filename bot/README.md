...
`interval.test.ts` — лимит 24 ч и окно следующей закупки (включая определение
понедельника по МСК). Эти модули намеренно не ходят в БД, поэтому тесты гоняются
в CI без секретов и без сети. Интерфейс бота не покрываем.
