...
`interval.test.ts` — лимит 24 ч и окно следующей закупки, `weekly.test.ts` —
определение понедельника по МСК. Эти модули намеренно не ходят в БД,
...
└── src/
    ...
    └── handlers/
        ...
        ├── weekly.ts       итог недели по понедельникам (+ .test)
