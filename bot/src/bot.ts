...
export function createBot(token: string = env.botToken): Bot<AppContext> {
  ...
  bot.on('callback_query:data', async (ctx) => {
    ...
    try {
      ...
      // ⚠️ НЕ пишем «попробуй заново». Запись покупки идёт ПЕРВОЙ, а уже после
      // неё — закрытие заказа, подсказка, ответ и пост в группу. Упади что-то
      // на этих шагах, покупка УЖЕ в базе: «попробуй заново» заставило бы
      // оператора ввести её повторно и породило бы дубль в данных.
      // Отсюда единственная честная формулировка — «проверь, прошло ли».
      await ctx.answerCallbackQuery({ text: '⚠️ Ошибка — проверь /recent' }).catch(() => {});
      await ctx
        .reply(
          '⚠️ Что-то пошло не так.\n\n' +
            'Запись МОГЛА пройти — сначала проверь «📋 Последние» (/recent) ' +
            'и только потом вводи заново, иначе получится дубль.',
        )
        .catch(() => {});
    }
  });
  ...
  bot.catch(async (err) => {
    ...
    // Раньше об ошибке узнавал только модератор, а оператор видел тишину:
    // нажал кнопку — ничего не произошло. Он либо решит, что не сохранилось,
    // и введёт повторно (дубль), либо что сохранилось (пропуск). Говорим ему
    // прямо. Ошибка отправки здесь не важна — молчание и так худший случай.
    await err.ctx
      .reply(
        '⚠️ Что-то пошло не так, модератору отправлено уведомление.\n\n' +
          'Если вводил закупку — проверь «📋 Последние» (/recent) перед тем, ' +
          'как повторять: запись могла пройти.',
      )
      .catch(() => {});
  });
