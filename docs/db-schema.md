...
| `0012_bot_sessions_rls.sql` | RLS на `bot_sessions` (забыли в `0003`) — ⏳ ждёт применения |
...
**Состояние на 12.09.2026 (проверено аудитом):** RLS включён на **пяти** рабочих
таблицах (`users`, `purchase_categories`, `phones`, `purchases`, `order_queue`),
**политик нет ни у одной**, а на `bot_sessions` RLS **выключен** — забыли в
миграции `0003`. Исправляется миграцией `0012` (⏳ ждёт применения).
